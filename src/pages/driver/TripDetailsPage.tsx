...
export default function TripDetailsPage() {
    ...
    const handleStatusUpdate = async (status: string, data?: any) => {
        ...
        try {
            ...
            const updatedTrip = await apiMethods.updateTripStatus(trip._id, status, data);
            setTrip(updatedTrip);
    ...
    if (loading) return <div className="p-8 text-center bg-muted/20 mt-8 rounded-lg animate-pulse h-96 mx-8" />;
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl space-y-6">
            ...
            <div className="space-y-4">
                <Button variant="ghost" className="pl-0 hover:pl-2 transition-all" asChild>
                    <Link to={-1 as any}> <ArrowLeft className="mr-2 h-4 w-4" /> Back to Dashboard</Link>
                </Button>

                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <h1 className="text-3xl font-bold tracking-tight">Trip {trip.tripNumber}</h1>
                            <Badge variant="outline" className="text-sm uppercase tracking-wider">{trip.status.replace("_", " ")}</Badge>
                        </div>
            ...
            <Card className="overflow-hidden border-2 border-primary/10">
                ...
                <CardContent className="pt-6 grid md:grid-cols-2 gap-8">
                    <div className="space-y-1">
                        <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Origin</p>
                    ...
                    <div className="space-y-1">
                        <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Destination</p>
                        <p className="text-lg font-semibold flex items-start gap-2">
                    ...
                    <div className="md:col-span-2 h-48 bg-muted/20 rounded-lg border-2 border-dashed border-muted flex flex-col items-center justify-center text-muted-foreground">
                        <MapPin className="h-8 w-8 mb-2 opacity-50" />
                        <span className="text-sm font-medium">Map visualization unavalailable</span>
                    </div>
                </CardContent>
            </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex justify-between items-center py-2 border-b">
                            ...
                            <span className="font-medium">{truck?.brand} {truck?.vehicleModel} ({truck?.licensePlate})</span>
                        </div>
                        <div className="flex justify-between items-center py-2 border-b">
                            <span className="text-muted-foreground">Trailer</span>
                            <span className="font-medium">
                                {trailer ? `${trailer.type} (${trailer.licensePlate})` : "None Assigned"}
                            </span>
                        </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex justify-between items-center py-2 border-b">
                            ...
                            <span className="font-medium">{new Date(trip.scheduledDate).toLocaleTimeString()}</span>
                        </div>
                        {trip.startDate && (
                            <div className="flex justify-between items-center py-2 border-b">
                                <span className="text-muted-foreground">Started</span>
                                <span className="font-medium">{new Date(trip.startDate).toLocaleString()}</span>
                            </div>
                        )}
                        {trip.endDate && (
                            <div className="flex justify-between items-center py-2 border-b">
                                <span className="text-muted-foreground">Completed</span>
                                <span className="font-medium">{new Date(trip.endDate).toLocaleString()}</span>
                            </div>
                        )}
                        {trip.notes && (
                            <div className="pt-2">
